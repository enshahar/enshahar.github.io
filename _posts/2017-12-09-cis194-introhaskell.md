...
summary:    "하스켈을 소개하고 기본 타입, 산술연산, `if`식, 함수정의, 순서쌍, 인자가 여럿 있는 함수, 리스트 등에 대해 설명한다."
...
> ##### 문자(알파벳이나 로만 이뤄진 이름의 연산자와 기호로만 이뤄진 이름의 연산자의 차이
> 
> 콘스를 지칭하면서 `(:)`라고 이야기했지만, 실제 코드에서 리스트를 만들 땐 `1 : []` 처럼 양 끝의 괄호 없이 `:`만을 사용했다. 하스켈에서는 기호로만 이뤄진 연산자를 함수처럼 정의할 때는 `()`로 둘러싸야 하며, 이렇게 기호로만 이뤄진 연산자를 특별히 백틱(`\``)없이 중위 연산자로 사용할 수 있다. 그러나 기호로만 이뤄진 연산자를 문자로 이뤄진 함수처럼 전위 연산자 형태로 호출하려면 괄호로 감싸야 한다. 즉 다음 두 호출은 같은 호출이다.
>```haskell
>1 : []
>(:) 1 []
>```
> 반대로, 문자로만 이뤄진 이름의 함수의 경우 괄호로 감싸지 않고 함수를 직접 전위 호출로 호출할 수 있지만, 중위 호출로 호출할 때는 백틱이 필요하다. 즉, `(:)`와 같은 역할을 하는 `cons`라는 함수를 정의했다면 다음 두 호출은 같다.
>```haskell
>cons 1 []
>1 `cons` []
>```
...
`n`개와 `a`, `b`, `c` 기둥이 있을 때 `a`에서 `c`로 디스크를 모두 이동(이때 `b`를 임시 저장 기둥으로 사용한다) 과정은 다음과 같은 재귀 알고리즘을 정의할 수 있다. 이런 경우는 `hanoi n "a" "c" "b"`라고 함수를 호출한 경우에 해당한다.
