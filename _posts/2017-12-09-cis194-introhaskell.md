...
- `Int`는 최소 `-2^29`부터 `+2^29`의 범위를 보장하는 정수타입이다. 구현(하부 컴퓨터의 아키텍처)에 따라 가능한 값의 범위가 달라질 수 있다. 
...
- `mod`를 `mod 19 3`처럼 사용할 때는 `mod`라는 이름을 그대로 사용하지만 ```19 `mod` 3``` 처럼 사용할 때는 백틱("`")으로 감다.
- 음수를 귀찮게 괄호("()")로 감쌌다.
